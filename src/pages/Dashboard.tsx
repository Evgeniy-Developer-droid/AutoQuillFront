  ...
  return <>
    <div className="flex flex-col md:flex-row justify-between p-4">
      ...
      <div className="bg-red-300 text-black p-4 m-2 rounded-lg flex-1">
        <div className={"flex flex-row justify-between items-center h-full"}>
            <h2 className="text-lg font-bold">{t("AI Generated Posts")}</h2>
    ...
    <div className="flex flex-col md:flex-row justify-between p-4">
        ...
        <ul>
            {data?.last_channels.map((channel, index) => (
                <div key={index} className={"p-2 m-2 bg-gray-700 rounded-lg flex flex-col items-center"}>
                    ...
                    <p>{t("Posts")} <span className={"badge badge-primary"}>{channel.posts_count}</span></p>
                </div>
            ))}
        </ul>
      </div>
      <div className="bg-gray-600 p-4 m-2 rounded-lg flex-1">
        <h2 className="text-lg font-bold">{t("Last 10 Posts")}</h2>
        <ul>
      ...
      <div className="bg-gray-600 p-4 m-2 rounded-lg flex-1">
        <h2 className="text-lg font-bold">{t("Last 10 Logs")}</h2>
        <ul>
